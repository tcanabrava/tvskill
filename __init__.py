...
__author__ = "tcanabrava"


class BBCSkill(MycroftSkill):
    def __init__(self):
        super(BBCSkill, self).__init__(name="BitChuteSkill")
    ...
    def initialize(self):
        LOG.info("Iniciando o coiso da skill\n")

    @intent_handler(IntentBuilder('SearchIntent').require('BitChuteSearchPageKeyword'))
    def search_videos(self, message):
        LOG.info("Called" + str(message))
    ...
    def clear_previous_video(self):
        ...
        self.gui["videoThumb"] = ""
        self.gui.show_pages(
            ["BitchutePlayer.qml", "RelatedPage.qml"], 0, override_idle=True
        )

    def get_play_video(self, query):
        LOG.info("Getting the video " + query)
        self.clear_previous_video()
        url = "https://www.nbcnews.com/search?q=".format(
            query
        )
        LOG.info("Tentando pegar o site " + url)
        response = requests.get(url)

        LOG.info("E pegou a resposta" + str(response))
        html = response.text
        LOG.info(html)
        ...
        try:
            ...
            if playable_url is not None:
                playableObj = {
                    "videoID": processed_result[0]["videoID"],
                    "videoTitle": processed_result[0]["videoTitle"],
                    "videoImage": processed_result[0]["videoImage"],
                    "videoChannel": processed_result[0]["videoChannel"],
                    "videoViews": processed_result[0]["videoViews"],
                    "videoUploadDate": processed_result[0]["videoUploadDate"],
    ...
    #     bitchutepause = (
    #         IntentBuilder("BitChutePauseKeyword")
    #         .require("BitChutePauseKeyword")
    #         .build()
    #     )
    #     self.register_intent(bitchutepause, self.bitchutepause)

    #     bitchuteresume = (
    #         IntentBuilder("BitChuteResumeKeyword")
    #         .require("BitChuteResumeKeyword")
    #         .build()
    #     )
    ...
    #     bitchutelauncherId = (
    #         IntentBuilder("BitChuteLauncherId")
    #         .require("BitChuteLauncherIdKeyword")
    #         .build()
    #     )
    ...
    #     self.gui.register_handler("BitChuteSkill.SearchLive", self.searchLive)

    #     self.gui.register_handler(
    #         "BitChuteSkill.RefreshWatchList", self.refreshWatchList
    #     )
    #     self.gui.register_handler(
    #         "BitChuteSkill.RelatedWatchList", self.bitchuterelatedpage
    #     )
    ...
    #     LOG.info("TCanabrava - Starting to search for the video")
    #     query = quote(text)
    ...
    #         if (
    #             "googleads" not in vid["href"]
    #             and not vid["href"].startswith(u"/user")
    #             and not vid["href"].startswith(u"/channel")
    #         ):
    ...
    #     LOG.info("TCanabrava - Search here")
    #     videoList = []
    #     videoList.clear()
    #     videoPageObject = {}
    #     try:
    ...
    #         url = "https://search.bitchute.com/renderer?use=bitchute-json&name=Search&login=bcadmin&key=7ea2d72b62aa4f762cc5a348ef6642b8&query={0}".format(
    #             query
    #         )
    #         response = requests.get(url)
    #         html = response.text
    ...
    #     getVideoDetails = zip(
    #         soup.findAll(attrs={"class": "ossfieldrdr1"}),
    #         soup.findAll("img"),
    #         soup.findAll(attrs={"class": "ossfieldrdr4"}),
    #         soup.findAll(attrs={"class": "ossfieldrdr6"}),
    #     )
